--[[
Game : https://www.roblox.com/games/12914664474
...
---- vars ----
kVars = {}
kVars.WindowName = "Mushroom Simulator GUI"
...
----========== page main ==========----
---- Farm ----
sectionFarm:Create("Toggle", "Tp to collectables",function(bool)
...
function fCollect()
    spawn(function()
        while kVars.boolCollect do
            wait()
            for i,v in pairs(game:GetService("Workspace").Spawnables.Main:GetChildren()) do
                wait()
                -- Script generated by SimpleSpy - credits to exx#9394
                kVars.hrp.CFrame = v.Main.CFrame + Vector3.new(0, 2, 0)
                local args = {
                    [1] = tonumber(v.name),
                    [2] = workspace.SpawnZones.Main
                }

                game:GetService("ReplicatedStorage").Remotes.CollectSpawnable:FireServer(tonumber(v.name), workspace.SpawnZones.Main)
                ...
                if kVars.boolCollect == false then break end
            end
        end
    end)
end
...
function fSell()
    spawn(function()
        while kVars.boolSell do
            wait()
            local closest = game:GetService("Workspace").Zones
            local last = math.huge
            for i,v in pairs(game:GetService("Workspace").Zones:GetChildren()) do
                if v.name == "Sell" and v:FindFirstChild("Main") and (v:FindFirstChild("Main").Position - kVars.hrp.Position).magnitude < last then
                    closest = v:FindFirstChild("Main")
                    last = (v:FindFirstChild("Main").Position - kVars.hrp.Position).magnitude
                end
            end

            if closest.name ~= "Zones" then
                firetouchinterest(kVars.hrp, closest, 0)
                firetouchinterest(kVars.hrp, closest, 1)
            end
            wait(.1)
        end
    end)
