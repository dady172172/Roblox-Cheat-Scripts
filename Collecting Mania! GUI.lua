--[[
Game : https://www.roblox.com/games/6482128340
...
---- Variables ----
kVars = {}
kVars.WindowName = "Collecting Mania! GUI"
...
sectionAutoFarm:addToggle("Collect", false, function(bool)
...
function fCollect()
    spawn(function()
        while kVars.boolCollect do
            ...
            for i,v in pairs(game:GetService("Workspace").CreatedCoins:GetChildren()) do
                if kVars.boolCollect == false then return end
                if v.PrimaryPart and areaName.PrimaryPart then
                    firetouchinterest(v.PrimaryPart, areaName.cc1, 0)
                    --areaName:MoveTo(v.PrimaryPart.Position)
                    --anotherway to do it
                    --[[
                        while wait() do
                            for i,v in pairs(game:GetService("Workspace").CreatedCoins:GetChildren()) do
                                game:GetService("ReplicatedStorage").Events.ClientEvents.CollectCoin:Fire(v)
                                v:Destroy()
                            end
                        end
                    ]]--
...
sectionAutoFarm:addToggle("Screen Spam", false, function(bool)
    if bool then 
        game:GetService("Players").LocalPlayer.PlayerGui.Main.NotificationsFrame.Visible = false
    else
        game:GetService("Players").LocalPlayer.PlayerGui.Main.NotificationsFrame.Visible = true
    end
 end)
