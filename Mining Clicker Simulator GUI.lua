...
---- section eggs ----
kVars.eggsList = {
    [1] = {name = "Starter Egg - $320", key = "Starter"},
    [2] = {name = "Spotted Egg - $1.2K", key = "Spotted"},
    [3] = {name = "Floral Egg - $10K", key = "Floral"},
    [4] = {name = "Desert Egg - $100K", key = "Desert"},
    [5] = {name = "Snow Egg - $1M", key = "Snow"},
    [6] = {name = "Cave Egg - $7.5M", key = "Cave"},
    [7] = {name = "Ocean Egg - $20M", key = "Ocean"},
    [8] = {name = "Jungle Egg - $120M", key = "Jungle"},
    [9] = {name = "Volcano Egg - $400M", key = "Volcano"},
    [10] = {name = "Space Egg - $1.8B", key = "Space"},
    [11] = {name = "Undead Egg - $15B", key = "Undead"},
    [12] = {name = "Heavenly Egg - $150B", key = "Heavenly"},
    [13] = {name = "Devil Egg - $500B", key = "Devil"}
}
kVars.eggsListNames = {}
for i,v in ipairs(kVars.eggsList) do
    table.insert(kVars.eggsListNames, v)
end
kVars.selectedEgg = "Starter Egg - $320"
sectionEggs:addDropdown("Select and Egg", kVars.eggsListNames, function(value)
    kVars.selectedEgg = value
end)
kVars.openAmount = 1
sectionEggs:addToggle("Open 3 Eggs", false, function(bool)
    if bool then
        kVars.openAmount = 3
    else
        kVars.openAmount = 1
...
sectionEggs:addToggle("OpenEgg", false, function(bool)
    ...
    spawn(function()
        while kVars.boolOpenEgg do
            ...
            for i,v in pairs(kVars.eggsList) do
                if v.name == kVars.selectedEgg then
                    game:GetService("ReplicatedStorage").Remotes.buyEgg:InvokeServer(v.key,tonumber(kVars.openAmount))
                    break
                end
            end
        end
