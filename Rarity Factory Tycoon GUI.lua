--[[
Game : https://www.roblox.com/games/10919241870
...
---- vars ----
kVars = {}
kVars.WindowName = "Rarity Factory Tycoon GUI"
...
sectionFarm:Create("Toggle", "Collect Balls",function(bool)
    kVars.boolCollectBalls = bool
    if bool then
        fCollectBalls()
    end
end,{default = kVars.boolCollectBalls})

function fCollectBalls()
    spawn(function()
        while kVars.boolCollectBalls do
            wait()
            for i,v in pairs(kVars.plrTycoon.Drops:GetChildren()) do
                firetouchinterest(v, game.Players.LocalPlayer.Character.Pickup, 0)
                firetouchinterest(v, game.Players.LocalPlayer.Character.Pickup, 1)              
            end
        end
    end)
end
...
function fSell()
    spawn(function()
        while kVars.boolSell do
            wait()
            firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, kVars.plrTycoon["Orb Processor"].Model.Deposit.Button, 0)
            firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, kVars.plrTycoon["Orb Processor"].Model.Deposit.Button, 1)
        end
    end)
end

sectionFarm:Create("Toggle", "Auto Buy Upgrades",function(bool)
    kVars.boolTouchUpgrades = bool
    if bool then
        fTouchUpgrades()
    end
end,{default = kVars.boolTouchUpgrades})

function fTouchUpgrades()
    spawn(function()
        while kVars.boolTouchUpgrades do
            wait()
            for i,v in pairs(kVars.plrTycoon.Buttons:GetChildren()) do
...
sectionFarm:Create("Toggle", "Auto Finish Parkour",function(bool)
    kVars.boolFinishParkour = bool
    if bool then
        fFinishParkour()
    end
end,{default = kVars.boolFinishParkour})

function fFinishParkour()
    spawn(function()
        while kVars.boolFinishParkour do
            wait()
            firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, game:GetService("Workspace").obby["2x Cash for 1 Minute"].Button, 0)
            firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, game:GetService("Workspace").obby["2x Cash for 1 Minute"].Button, 1)
        end
    end)
end
...
function fRebirth()
    spawn(function()
        while kVars.boolRebirth do
            wait()
            firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, kVars.plrTycoon.Rebirth.Button, 0)
            firetouchinterest(game.Players.LocalPlayer.Character.HumanoidRootPart, kVars.plrTycoon.Rebirth.Button, 1)
        end
    end)
end
